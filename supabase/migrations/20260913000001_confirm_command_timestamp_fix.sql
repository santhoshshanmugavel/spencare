-- Spencare — confirm_command timestamp fix.
--
-- Fixes two bugs introduced in 20260911000001_confirm_command_v2.sql:
--
-- BUG 1: All occurredAt casts used ::date, which strips time.
--   e.g. '2026-09-12T20:20:00+05:30'::date = '2026-09-12'
--   then stored as '2026-09-12 00:00:00+00' = 5:30 AM IST.
--   Fix: cast to ::timestamptz to preserve the full instant.
--
-- BUG 2: createTransaction and updateTransaction cases never passed
--   p_item_name. The RPC defaulted it to NULL, wiping item_name on every
--   MCP-confirmed create or update.
--   Fix: pass p_item_name => v_payload->>'itemName' in both cases.
--
-- ROLLBACK: re-run 20260911000001_confirm_command_v2.sql to revert.
  ...
  case v_confirmation.command_type
    ...
    when 'createTransaction' then
      v_txn := create_transaction(
        ...
        p_occurred_at => (v_payload->>'occurredAt')::timestamptz,
