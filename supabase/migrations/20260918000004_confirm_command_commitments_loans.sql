-- Spencare — confirm_command v4: Planned Commitments and Loans.
--
-- Adds 'bike' to the loan_type enum (spec requirement).
-- Extends confirm_command with commitment management (createCommitment,
-- updateCommitment, deleteCommitment, pauseCommitment, resumeCommitment,
-- reserveOccurrence, skipOccurrence, markOccurrencePaid) and loan
-- management (createLoan, updateLoan, deleteLoan).
--
-- All commitment operations are PLANNING + REMINDER only. No money is
-- moved, no account balances change. The safety invariant from Phase 18:
-- Spencare must not silently move money between accounts.
--
-- SECURITY: every branch checks user_id = p_user_id before acting.
-- This function runs as SECURITY DEFINER; per-row checks are defense-in-depth.

-- Add 'bike' to loan_type enum
alter type loan_type add value if not exists 'bike';
...
begin
  ...
  case v_confirmation.command_type
    ...
    -- ── Original + Phase 6 + Phase 18 plan types (PRESERVED EXACTLY) ──────
    ...
    when 'createCommitment' then
      insert into planned_commitments (
        ...
        first_saving_date, funding_account_id, tenure_type, tenure_payments,
        tenure_end_date, notes
      ) values (
        p_user_id,
        v_payload->>'name',
        (v_payload->>'categoryId')::uuid,
      ...
      insert into planned_commitment_occurrences (
        commitment_id, user_id, due_date, amount_minor,
        ...
        (v_payload->>'nextPaymentDate')::date,
        (v_payload->>'amountMinor')::bigint,
        case when v_payload->>'savingCadence' is null then (v_payload->>'amountMinor')::bigint else 0 end,
        'upcoming'
      );
