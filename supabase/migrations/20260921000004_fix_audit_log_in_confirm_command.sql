-- Fix: confirm_command has been crashing on every call since 2026-09-20 because
-- the audit_log INSERT at the end references columns that do not exist
-- (event_type, payload). The real columns are action, entity_type, entity_id, after.
-- This rolls back the entire PL/pgSQL block on every successful mutation,
-- so all pending_confirmations stay stuck as 'pending' and are never confirmed.
--
-- Introduced by: 20260920000007_confirm_command_mark_loan_paid.sql
-- Carried forward in: 20260921000003_fix_confirm_command_p0.sql (same bad INSERT)
-- Fix: replace the broken INSERT with correct column names.
...
begin
  ...
  -- FIXED: use actual audit_log columns (action, entity_type, entity_id, after).
  -- The broken versions used event_type and payload (do not exist), which caused
  -- Postgres to raise 42703, rolling back the entire transaction on every call.
